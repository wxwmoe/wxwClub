...
// web 请求也会走 resolver，DNS 争用和刷新等待要在这一侧留下同样的计数，
// 否则日志里只看得见 worker 那一半，冷缓存被 FPM 抢走的情况永远对不上账
...
// 库里的结构与这份代码要求的版本不一致时，整个入口先挡住。只挡 Club_Push_Activity()
// 是不够的：入站活动写进本地状态之后才报错的话，对端重放会形成半处理。
// 503 + Retry-After 是明确的「稍后再来」，换成 4xx 对端就当终局拒绝，那条活动永远丢了
// 库比代码新同样不能放行：回滚或滚动部署中的旧代码不认识新结构，继续写会损坏数据。
