...
/* endpoints 加 idle_since：控制行变空的时刻。回收要等它空置够久才动手，
 * 否则大群组每投一轮就让名下几千行同时变空，几分钟后又原样建回来。
 *
 * 顺带清掉第 2 版留在 meta 里的断点。断点状态的生命周期不该超过它所属的那一版，
 * 但第 2 版已经在库里生效过，那个文件再也不会执行，这条只能补在这里。 */

function Club_Migrate_3() {
    global $db;
    // 定义不对就改过来：默认值是硬要求，Club_Endpoint_Upsert 的 insert 分支
    // 根本不写这一列，新排上的 endpoint 全靠它落成 0
    ...
    // 存量的空行一条起点都没有，不回填就永远过不了宽限期，回收从此不工作。
    // 条件本身幂等：中途崩了重跑只会命中还没补过的那些
...
function Club_Migrate_3_Validate() {
    ...
    Club_Migrate_Assert((string)$info['default'] === '0', 'endpoints.idle_since defaults to zero',
        ['actual' => $info['default']]);
    // next_at 为空和 idle_since 非零必须同进同出：少了前者回收永远等不到宽限期，
    // 少了后者 endpoints_idle 会把在排队的行也算进去
