...
| --- | --- | --- |
| `pure` | no | Table-driven pure functions: normalization, the SSRF address rules, DoH parsing, date/digest, every state decision, and `Club_Config_Validate` |
| `state` | yes | The same decisions written to disk, plus lease fencing, blacklist entry and exit, reconciliation and reclaim |
| `schema` | yes | Fresh import, upgrade from every fixture in `tests/fixtures/schema/`, restart at every intermediate version, second-run no-op |
...
- `maintain` is fixed at one **per master** and deliberately not configurable — log rotation, reconciliation and blacklist cleanup are site-wide work, and running N copies just does the same thing N times. Across masters the MySQL named lock `wxwclub_maintain:<md5 of the database name>` picks the one that actually works (hashed in PHP, not by the server's `MD5()`: a lock name caps at 64 characters and a database name alone may already fill it, and PHP knows the name it put in the DSN anyway): taken non-blocking at the top of every round, never released, so it lives and dies with the connection and a master that goes away hands the work over within one round. The others idle. Scale with `worker.delivery` / `worker.probe`; a second master buys nothing for maintenance. **The maintenance queue never resolves a hostname, never makes an HTTP request, and never delivers or probes.** It does own the `dns` table's expiry sweep, which is a row delete and touches no resolver. Mixing the two means a sustained backlog starves maintenance forever.
...
4. Results land only through `Club_Endpoint_Result`, which locks the row by token, so a stale owner cannot write a single field.
