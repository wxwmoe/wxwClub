...
- A SQL statement stays on one line and splits only when it would otherwise overrun the column ceiling. When it does split, break at clause boundaries and spread the clauses evenly over the fewest lines that hold them — a statement needing three lines reads as three clause groups, not two full lines and a remainder. A `create table` body is the exception: one column per line is a table, not a wrapped line.
